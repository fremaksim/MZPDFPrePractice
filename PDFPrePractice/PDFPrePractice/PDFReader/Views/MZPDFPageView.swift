...
internal final class MZPDFPageView: UIScrollView {
    ...
    private weak var pageViewDelegate: MZPDFPageViewDelegate?
    ...
    init(frame: CGRect, document: MZPDFDocument, pageNumber: Int, backgroundImage: UIImage?, pageViewDelegate: MZPDFPageViewDelegate?) {
//        guard let pageRef = document.coreDocument.page(at: pageNumber + 1) else { fatalError() }
        guard let pageRef = document.documentRef?.page(at: pageNumber + 1) else {fatalError("cgPDFRef cannot nil")  }
        ...
        tiledPDFView = MZPDFTiledView(frame: contentView.bounds, scale: scale, newPage: pdfPage)
...
extension MZPDFPageView: UIScrollViewDelegate {
